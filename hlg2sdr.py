...
from utils.transfer import eotf_hlg, oetf_sdr
...
def process_video(input_path, output_path):
    ...
    print(f"Converting HLG -> SDR: {input_path} -> {output_path}...")
    ...
    for frame in input_container.decode(in_stream):
        ...
        # B. Linearize (HLG EOTF) -> Returns Nits
        rgb_scene_linear = eotf_hlg(rgb_nonlinear)

        # C. Apply HLG System Gamma (OOTF)
        # This converts "Scene Light" to "Display Light"
        # Gamma 1.2 is standard for HLG on a 1000 nit display
        # Formula: Display = Scene ^ 1.2
        rgb_display_linear = np.power(rgb_scene_linear, 1.2)

        # D. Convert Primaries (Rec.2020 -> Rec.709)
        rgb_linear_709 = linear_2020_to_linear_709(rgb_display_linear)
        rgb_linear_709 = np.maximum(rgb_linear_709, 0.0)  # Clip negatives

        # E. Peak Luminance Scaling
        # We assume HLG represents 1000 nits peak.
        # We want to map that range into the Tone Mapper.
        # This acts as our "Exposure" control.
        # 1000 nits / 100 nits (SDR Target) = 10.0 max value conceptually
        rgb_linear_709 = rgb_linear_709 * (1000.0 / 100.0)
...
output_path = os.path.join(output_dir, "test_hlg2sdr.mp4")
