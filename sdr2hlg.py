...
from utils.transfer import eotf_sdr, oetf_hlg
...
def process_video(input_path, output_path):
    ...
    print(f"Converting SDR -> HLG: {input_path} -> {output_path}...")
    ...
    for frame in input_container.decode(in_stream):
        ...
        # D. Scale for HLG (Inverse Tone Map)
        # Map SDR White to 203 Nits (ITU Ref White)
        rgb_linear_hlg_normalized = rgb_linear_2020 * (203.0 / 1000.0)

        # E. Apply HLG OETF (Linear -> HLG Signal)
        rgb_hlg = oetf_hlg(rgb_linear_hlg_normalized)
...
output_path = os.path.join(output_dir, "test_sdr2hlg.mp4")
